...
| 类型 | 重写方法 | 用途 |
|------|---------|------|
| `TickSystem` | `OnTick(World, float)` | 全局逻辑、无查询、自定义遍历 |
| `ChunkSystem` | `CreateQuery()` + `ForEachChunk(World, Chunk, float)` | 批量操作、列预取、Chunk 级跳过 |
| `EntitySystem` | `CreateQuery()` + `ForEachEntity(World, Chunk, int, float)` | 逐实体处理（最常用） |

系统通过 `ticker.Register<T>()` 注册到指定 Ticker，由用户决定在 Unity 哪个生命周期驱动。
`runOnce: true` 可让系统只执行一次后自动移除（替代原 InitSystem）。

### 4.2 EntitySystem（最常用）

框架遍历匹配的 Chunk 和实体，自动跳过延迟销毁的实体：
...
public class DamageSystem : EntitySystem
{
    ...
    protected override void ForEachEntity(World world, Chunk chunk, int indexInChunk, float deltaTime)
    {
        ref var hp = ref chunk.GetComponent<Health>(indexInChunk);
        hp.Current -= 10 * deltaTime;
        if (hp.Current <= 0)
            world.AddComponent(chunk.GetEntity(indexInChunk), default(DeadTag));
    }
}
```
...
框架遍历匹配的 Chunk，用户自行遍历实体。适合批量操作、列预取、Chunk 级跳过：

```csharp
public class PhysicsSystem : ChunkSystem
{
    protected override EntityQuery CreateQuery()
    {
        return new EntityQuery(
            all: new ComponentMask().With<Position>().With<Velocity>()
        );
    }

    protected override void ForEachChunk(World world, Chunk chunk, float deltaTime)
    {
        // 预取列数据，热路径更高效
...
可选查询过滤，框架收集匹配的 Chunk 列表后调用 OnTick。适合全局逻辑、事件处理：

```csharp
public class InputSystem : TickSystem
{
    // 可选：重写 CreateQuery 过滤特定 Chunk
    protected override EntityQuery CreateQuery()
    {
        return new EntityQuery(all: new ComponentMask().With<InputData>());
    }

    protected override void OnTick(World world, IReadOnlyList<Chunk> chunks, float deltaTime)
    {
        // chunks 匹配的 Chunk 列表，自行决定如何处理
    }
}
```
...
唯一需要注意的是：不要在 `EntitySystem.ForEachEntity`、`ChunkSystem.ForEachChunk` 或 `EcsAPI.Query(...).AsRows()` 的遍历过程中直接创建/销毁实体、添加/移除组件。遍历中需要结构变更时，先记录到列表或 `EntityCommandBuffer`，遍历结束后再执行。
...
```csharp
public class SpawnPlayerSystem : TickSystem
{
    protected override void OnTick(World world, float deltaTime)
    {
...
public class DeathSystem : EntitySystem
{
    ...
    protected override void ForEachEntity(World world, Chunk chunk, int indexInChunk, float deltaTime)
    {
        ...
        {
            // 收集命令，不在遍历中直接修改结构
            ECB.AddComponent(chunk.GetEntity(indexInChunk), default(DeadTag));
        }
    }
}
```
...
Archetype.Layout (O(1) 查找) → Chunk[] → ChunkColumn<T>
  ↓ 遍历
EntitySystem / ChunkSystem / TickSystem
```
